...
class GB_JSON_API {
	...
	function admin_menu() {
		add_options_page( 'GoodBarber Api Settings', 'GoodBarber Api', 'manage_options', 'gb-json-api', array(
			&$this,
			'admin_options'
		) );
	}
	...
	function admin_options() {
		...
		<div>
			<form action="options-general.php?page=gb-json-api" method="post">
				<style type="text/css">  #goodbarber {
						background-color: #EDF2F2 !important;
						color: #222222 !important
					} </style>
				<div id='goodbarber'>
					<h1> GoodBarber Api</h1>
					<?php _e( 'A JSON-API fork used to synchronize your GoodBarber app with your wordpress.' ) ?>
					<br/><br/>
					...
					<?php _e( 'If you want to let your users post comments to your website right from your GoodBarber mobile app, check the box below.' ) ?>
					<br/><br/>
					<input name='is_comments_enabled' type="checkbox" <?php if ( $ice == 'True' ) {
						echo 'checked="checked"';
					} ?>><?php _e( ' Enable comment submission' ) ?><br/><br/>
					<h3><?php _e( 'Security management' ) ?> </h3>
					<?php _e( 'By default, GoodBarber Api secure mode is off. If you want to limit the access to this plugin to GoodBarber servers only, check the box below.' ) ?>
					<br/><br/>
					<?php $isme = get_option( 'is_secure_mode_enabled', 'False' ); ?>
					<input name='is_secure_mode_enabled' type="checkbox" <?php if ( $isme == 'True' ) {
						echo 'checked="checked"';
					} ?>><?php _e( ' Enable secure mode (IP restriction to GoodBarber servers only)' ) ?>
					<p class="submit"><input type="submit" class="button-primary" name="gb_submit"
											 value="<?php _e( 'Save Changes' ) ?>"/></p></div>
			</form>
		</div>
		<?php
	}
